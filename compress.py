...
def psnr_f(a: torch.Tensor, b: torch.Tensor) -> float:
    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
            ...
            if args.reconstruct is True:
                ...
                reconstruction, q_bpp = model(x_padded, writeout=False)
            else:
                # Perform entropy coding
                compressed_output = model.compress(x_padded)
                ...
                reconstruction = F.pad(
                        reconstruction, (-padding_left, -padding_right, -padding_top, -padding_bottom))
                #--------------------------------#
...
def main(**kwargs):
    ...
    parser.add_argument("-ckpt", "--ckpt_path", type=str, default='ms-ssim_lr_60.50/openimages_compression_2023_04_10_12_36_epoch80.pt', help="Path to model to be restored")
    ...
    parser.add_argument("-rc", "--reconstruct", help="Reconstruct input image without compression.", default=False) # action="store_true"
    parser.add_argument("-save", "--save", help="Save compressed format to disk.", default=True)
    parser.add_argument("-metrics", "--metrics", help="Evaluate compression metrics.", default=True)
