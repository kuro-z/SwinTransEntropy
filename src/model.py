...
class Model(nn.Module):
    ...
    def __init__(self, args, logger, storage_train=defaultdict(list), storage_test=defaultdict(list), model_mode=ModelModes.TRAINING, 
        ...
        self.Encoder = encoder.Encoder(C=self.args.latent_channels,)
    ...
    def distortion_loss(self, x_gen, x_real):
        ...
        #MS-SSIM
        sq_err = self.metric(x_gen, x_real, data_range=1)
        return sq_err
    ...
    def compression_loss(self, intermediates, hyperinfo):
        ...
        # # PSNR
        # distortion_loss = self.args.k_M * 255 ** 2 * self.distortion_loss(x_gen, x_real)
        # MS-SSIM
        distortion_loss = 1 - self.distortion_loss(x_gen, x_real)
        ...
        weighted_R_D_loss = self.args.k_M * distortion_loss + intermediates.n_bpp
        ...
        if (self.step_counter % self.log_interval == 1):
            ...
            self.store_loss('weighted_distortion', distortion_loss.item())
    ...
    def compress(self, x, silent=False):
        ...
        * Encode latents via entropy model derived from (mean, scale) hyperprior output.
        ...
        compression_output = self.Hyperprior.compress_forward(y, spatial_shape=x.size()[2:])
        attained_hbpp = 32 * len(compression_output.hyperlatents_encoded) / np.prod(spatial_shape)
        attained_lbpp = 32 * (len(compression_output.latents_encoded_a)+len(compression_output.latents_encoded_b)+len(compression_output.latents_encoded_c)+len(compression_output.latents_encoded_d)) / np.prod(spatial_shape)
        attained_bpp = 32 * ((len(compression_output.hyperlatents_encoded) +  
            (len(compression_output.latents_encoded_a)+len(compression_output.latents_encoded_b)+len(compression_output.latents_encoded_c)+len(compression_output.latents_encoded_d))) / np.prod(spatial_shape))
