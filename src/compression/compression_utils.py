...
def save_compressed_format(compression_output, out_path):
    ...
    with open(out_path, 'wb') as f_out:
        ...
        write_shapes(compression_output.latent_spatial_shape, f_out)  # (H,W)
        write_shapes(compression_output.spatial_shape, f_out)  # (H,W)
        write_shapes(compression_output.hyper_coding_shape, f_out)  # (C,H,W)
        write_shapes(compression_output.latent_coding_shape_a, f_out)  # (C,H,W)
        write_shapes(compression_output.latent_coding_shape_b, f_out)  # (C,H,W)
        write_shapes(compression_output.latent_coding_shape_c, f_out)  # (C,H,W)
        write_shapes(compression_output.latent_coding_shape_d, f_out)  # (C,H,W)
        ...
        # Write latents
        enc_latents_a = compression_output.latents_encoded_a
        write_num_bytes_encoded(len(enc_latents_a) * 4, f_out)
        message_to_bytes(f_out, enc_latents_a)
        f_out.write(_MAGIC_VALUE_SEP)

        enc_latents_b = compression_output.latents_encoded_b
        write_num_bytes_encoded(len(enc_latents_b) * 4, f_out)
        message_to_bytes(f_out, enc_latents_b)
        f_out.write(_MAGIC_VALUE_SEP)

        enc_latents_c = compression_output.latents_encoded_c
        write_num_bytes_encoded(len(enc_latents_c) * 4, f_out)
        message_to_bytes(f_out, enc_latents_c)
        f_out.write(_MAGIC_VALUE_SEP)

        enc_latents_d = compression_output.latents_encoded_d
        write_num_bytes_encoded(len(enc_latents_d) * 4, f_out)
        message_to_bytes(f_out, enc_latents_d)
        f_out.write(_MAGIC_VALUE_SEP)
...
def load_compressed_format(in_path):
    ...
    with open(in_path, 'rb') as f_in:
        ...
        latent_spatial_shape = read_shapes(f_in, 2)
        spatial_shape = read_shapes(f_in, 2)
        hyper_coding_shape = read_shapes(f_in, 3)
        latent_coding_shape_a = read_shapes(f_in, 3)
        latent_coding_shape_b = read_shapes(f_in, 3)
        latent_coding_shape_c = read_shapes(f_in, 3)
        latent_coding_shape_d = read_shapes(f_in, 3)
        ...
        # Read latents
        num_bytes_a = read_num_bytes_encoded(f_in)
        latents_encoded_a = message_from_bytes(f_in, num_bytes)
        assert f_in.read(4) == _MAGIC_VALUE_SEP  # assert valid file

        # Read latents
        num_bytes_b = read_num_bytes_encoded(f_in)
        latents_encoded_b = message_from_bytes(f_in, num_bytes)
        assert f_in.read(4) == _MAGIC_VALUE_SEP  # assert valid file

        # Read latents
        num_bytes_c = read_num_bytes_encoded(f_in)
        latents_encoded_c = message_from_bytes(f_in, num_bytes)
        assert f_in.read(4) == _MAGIC_VALUE_SEP  # assert valid file

        # Read latents
        num_bytes_d = read_num_bytes_encoded(f_in)
        latents_encoded_d = message_from_bytes(f_in, num_bytes)
        assert f_in.read(4) == _MAGIC_VALUE_SEP  # assert valid file
    ...
    compression_output = CompressionOutput(
        ...
        latent_coding_shape_d=latent_coding_shape_d)
